...
for sim in range(0, repeats):
    for chinch in range(N_intervalos):
        ...
        checkpoint_callback = ModelCheckpoint(
            filepath=f"black_hole_models/weights_inter={N_intervalos}_y={y_ini}_N={N_train_max}_sim={sim}.h5",
            monitor="loss",
            save_best_only=True,
        ...
        if chinch == (N_intervalos - 1):
            ...
            model.load_weights(
                f"black_hole_models/weights_inter={N_intervalos}_y={y_ini}_N={N_train_max}_sim={sim}.h5"
            )
    ...
    # * We save the PINN data trajectories
    ruta_chinchetas = (
        f"multiple_black_hole/pushpin_y0={y_ini}_Ninter={N_intervalos}_sim={sim}.txt"
    )
